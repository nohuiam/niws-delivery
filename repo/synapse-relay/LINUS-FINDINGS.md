# Linus Audit Findings: synapse-relay

**Auditor:** Claude Code Instance 8
**Date:** 2026-01-08
**Server:** synapse-relay
**Ports:** 3025 (UDP) / 8025 (HTTP) / 9025 (WS)
...
| ID | Issue | Notes |
|----|-------|-------|
| m1 | Buffer overflow potential | max_size config exists but enforcement unclear |
| m2 | Signal relay accepts from any source | No authentication on relay endpoint |
| m3 | Broadcast to unbounded WebSocket clients | May need connection limits |
...
app.post('/api/relay', relayLimiter, ...);
```
...
- Clean relay architecture with rule engine
- Buffer management for offline targets
