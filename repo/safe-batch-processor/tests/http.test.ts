...
vi.mock('../src/database/schema.js', () => ({
  getDatabase: () => ({
    getStats: () => ({ total_batches: 0, completed: 0, failed: 0 }),
    listBatchOperations: () => [],
    getBatchOperation: () => null,
    getBatchItems: () => []
  })
}));

vi.mock('../src/core/executor.js', () => ({
  executeBatch: async () => ({ batch_id: 'test-123', success: true, items_processed: 0 })
}));

vi.mock('../src/core/validator.js', () => ({
  validateBatch: () => ({ valid: true, errors: [] })
}));

vi.mock('../src/core/rollback.js', () => ({
  rollbackBatch: async () => ({ success: true, items_rolled_back: 0 })
}));

import { HttpServer } from '../src/http/server.js';

const TEST_PORT = 18022;
...
describe('HTTP Server', () => {
  ...
  it('returns health status', async () => {
    ...
    expect(data.server).toBe('safe-batch-processor');
  ...
  describe('CORS Security', () => {
    ...
    it('handles preflight OPTIONS requests for allowed origins', async () => {
      const response = await fetch(`http://localhost:${TEST_PORT}/api/batch`, {
        method: 'OPTIONS',
        headers: {
